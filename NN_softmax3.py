...
Data_set_list = ["aout1800000.txt" ] #each num.txt consists of num training data. We simulate for 1.4, 1.6, 1.8 millions here
...
def forward(x, weights, biases):#pass the data x from the input layer forwardly to other layers with the given weights and biases
	...
	for i in range(layernum):
		...
		if i < layernum-1:
			L[i+1] = tf.nn.sigmoid(z[i+1])														######################activation function here
...
def sgd(inputs, outputs, N_Samples, batch_size, initial_eta, N_input, N_output, TrainRatio, hidden_num, data_set):
	...
	for epoches in range(N_iteration):
		...
		if( epoches==3000):
			outfilename = "annoutsigmoid"+data_set[4:-4] + "_" + str(hidden_num[0]) + str(hidden_num[1]) +"_"+str(epoches)+".txt"
...
#					print(p+1)
		...
		if(epoches%100 == 0 and epoches > 0):#save the performance measures
			outputfilename = "performancesigmoid"+data_set[4:-4] + "_" + str(hidden_num[0]) + str(hidden_num[1]) +".txt"
