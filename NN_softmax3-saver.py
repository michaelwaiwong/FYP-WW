...
Data_set_list = ["aout100000.txt" ] #each num.txt consists of num training data. We simulate for 1.4, 1.6, 1.8 millions here
N_iteration = 1003#Number of iterations used for each simulation
...
def init(N_input, N_output, hidden_num):
	...
	for i in range(layernum):
		if(i == 0):# initialize the weights and bias in the first layer
			Weights['W1'] = tf.Variable(tf.truncated_normal([N_input, hidden_num[i]], stddev = 0.1),name='W1')
			Biases['B1'] = tf.Variable(tf.truncated_normal([hidden_num[i]])*0.01,name='B1')
		elif(i == layernum-1):# initialize the weights and bias in the output layer
			Weights['W'+str(layernum)] = tf.Variable(tf.truncated_normal([hidden_num[i-1], N_output], stddev = 0.1),name='W'+str(layernum))
			Biases['B'+str(layernum)] = tf.Variable(tf.truncated_normal([N_output])*0.01,name='B'+str(layernum))
		else: # initialize the weights and biases in the hidden layer
			Weights['W'+str(i+1)] = tf.Variable(tf.truncated_normal([hidden_num[i-1], hidden_num[i]], stddev = 0.1),name='W'+str(i+1))
			Biases['B'+str(i+1)] = tf.Variable(tf.truncated_normal([hidden_num[i]])*0.01,name='B'+str(i+1)),
...
def forward(x, weights, biases):#pass the data x from the input layer forwardly to other layers with the given weights and biases
	...
	for i in range(layernum):
		...
		if i < layernum-1:
			L[i+1] = tf.nn.relu(z[i+1])														######################activation function here
...
def sgd(inputs, outputs, N_Samples, batch_size, initial_eta, N_input, N_output, TrainRatio, hidden_num, data_set):
	...
	for epoches in range(N_iteration):
		...
		if(epoches%3 == 0):#print the accuracy after every 3 iterations
			print(epoches+1)
		...
		if(epoches==1000):
			outfilename = "annout2"+data_set[4:-4] + "_" + str(hidden_num[0]) + str(hidden_num[1]) +"_"+str(epoches)+".txt"
...
#					print(p+1)
		...
		if(epoches%100 == 0 and epoches > 0):#save the performance measures
			outputfilename = "performance2"+data_set[4:-4] + "_" + str(hidden_num[0]) + str(hidden_num[1]) +".txt"
	...
	with tf.Session() as sess:
		sess.run(tf.global_variables_initializer())
		save_path=saver.save(sess,"my_net18_350150/save_net.ckpt")
